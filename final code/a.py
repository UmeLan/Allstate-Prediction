...
dfa=df.drop('customer_ID').drop('shopping_pt').drop('time')\
  .drop('day').drop('state').drop('location')\
  .drop('age_oldest').drop('age_youngest')\
  .drop('B').drop('C').drop('D').drop('E').drop('F').drop('G')\
  .drop('plan').drop('hour')
...
model=DecisionTree.trainClassifier(trainData,numClasses=3,maxDepth=10,categoricalFeaturesInfo={},impurity='gini',maxBins=32)
...
def printMetrics(pred_and_label):
  ...
  print 'Precision of 2',metrics.precision(2)
