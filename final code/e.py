...
dfe=df.drop('customer_ID').drop('shopping_pt').drop('time')\
  .drop('day').drop('state').drop('location')\
  .drop('age_oldest').drop('age_youngest')\
  .drop('B').drop('C').drop('D').drop('A').drop('F').drop('G')\
  .drop('plan').drop('hour')

pd.DataFrame(dfe.take(5), columns=dfe.columns).transpose()
...
trainData,testData=labelData(dfe).randomSplit([0.8,0.2])

model=DecisionTree.trainClassifier(trainData,numClasses=2,maxDepth=3,categoricalFeaturesInfo={},impurity='gini',maxBins=32)
