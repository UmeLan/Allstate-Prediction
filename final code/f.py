...
dff=df.drop('customer_ID').drop('shopping_pt').drop('time')\
  .drop('day').drop('state').drop('location')\
  .drop('age_oldest').drop('age_youngest')\
  .drop('B').drop('C').drop('D').drop('E').drop('A').drop('G')\
  .drop('plan').drop('hour')

pd.DataFrame(dff.take(5), columns=dff.columns).transpose()
...
trainData,testData=labelData(dff).randomSplit([0.8,0.2])

model=DecisionTree.trainClassifier(trainData,numClasses=4,maxDepth=8,categoricalFeaturesInfo={},impurity='gini',maxBins=32)
