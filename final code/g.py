...
dfg=df.drop('customer_ID').drop('shopping_pt').drop('time')\
  .drop('day').drop('state').drop('location')\
  .drop('age_oldest').drop('age_youngest')\
  .drop('B').drop('C').drop('D').drop('E').drop('F').drop('A')\
  .drop('plan').drop('hour')

pd.DataFrame(dfg.take(5), columns=dfg.columns).transpose()
...
trainData,testData=labelData(dfg).randomSplit([0.8,0.2])

model=DecisionTree.trainClassifier(trainData,numClasses=4,maxDepth=5,categoricalFeaturesInfo={},impurity='gini',maxBins=32)
