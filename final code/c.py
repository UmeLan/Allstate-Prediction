...
dfc=df.drop('customer_ID').drop('shopping_pt').drop('time')\
  .drop('day').drop('state').drop('location')\
  .drop('age_oldest').drop('age_youngest')\
  .drop('A').drop('B').drop('D').drop('E').drop('F').drop('G')\
  .drop('plan').drop('hour')

pd.DataFrame(dfc.take(5), columns=dfc.columns).transpose()
...
trainData,testData=labelData(dfc).randomSplit([0.8,0.2])

model=DecisionTree.trainClassifier(trainData,numClasses=4,maxDepth=4,categoricalFeaturesInfo={},impurity='gini',maxBins=32)
