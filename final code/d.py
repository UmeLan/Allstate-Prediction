...
dfd=df.drop('customer_ID').drop('shopping_pt').drop('time')\
  .drop('day').drop('state').drop('location')\
  .drop('age_oldest').drop('age_youngest')\
  .drop('A').drop('B').drop('C').drop('E').drop('F').drop('G')\
  .drop('plan').drop('hour')
...
model=DecisionTree.trainClassifier(trainData,numClasses=4,maxDepth=6,categoricalFeaturesInfo={},impurity='gini',maxBins=50)
